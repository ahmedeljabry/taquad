        ...
        <div class="relative mx-auto flex flex-col gap-6 lg:max-w-6xl lg:flex-row lg:items-center lg:justify-between">
            <div class="space-y-4">
                <span class="inline-flex items-center gap-2 rounded-full bg-white/15 px-4 py-1 text-[12px] font-semibold uppercase tracking-[0.3em]">
                    ...
                    @lang('messages.t_seller_dashboard')
                </span>
                <div>
        ...
        <div class="rounded-2xl border border-white/60 bg-white/90 p-5 shadow-sm backdrop-blur-sm dark:border-zinc-700 dark:bg-zinc-900/70">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{{ money($earnings, settings('currency')->code, true) }}</p>
                </div>
        ...
        <div class="rounded-2xl border border-white/60 bg-white/90 p-5 shadow-sm backdrop-blur-sm dark:border-zinc-700 dark:bg-zinc-900/70">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{{ $awarded_projects }}</p>
                </div>
        ...
        <div class="rounded-2xl border border-white/60 bg-white/90 p-5 shadow-sm backdrop-blur-sm dark:border-zinc-700 dark:bg-zinc-900/70">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{{ $pending_orders }}</p>
                </div>
        ...
        <div class="rounded-2xl border border-white/60 bg-white/90 p-5 shadow-sm backdrop-blur-sm dark:border-zinc-700 dark:bg-zinc-900/70">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">{{ count($latest_messages) }}</p>
                </div>
            ...
            <div class="rounded-2xl border border-white/60 bg-white/90 p-5 shadow-sm backdrop-blur-sm dark:border-zinc-700 dark:bg-zinc-900/70 md:col-span-2 xl:col-span-4">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-xs font-semibold uppercase tracking-[0.25em] text-slate-500 dark:text-slate-400">
                            @lang('messages.t_monthly_proposals_progress_label')
                        </p>
                        <p class="mt-2 text-2xl font-bold text-slate-900 dark:text-white">
                            {{ $monthly_proposals_used }} / {{ $monthly_proposals_limit }}
                        </p>
                    </div>
                    <span class="flex h-11 w-11 items-center justify-center rounded-full bg-primary-500/10 text-primary-600 dark:text-primary-400">
                ...
                <div class="mt-4 space-y-2">
                    <div class="flex items-center justify-between text-[11px] font-semibold uppercase tracking-[0.2em] text-slate-500 dark:text-slate-400">
                        <span>@lang('messages.t_monthly_proposals_remaining'): {{ max($monthly_proposals_remaining, 0) }}</span>
                        <span>@lang('messages.t_monthly_proposals_reset', ['date' => format_date($monthly_proposals_reset_at->toDateTimeString(), config('carbon-formats.F_d,_Y'))])</span>
                    </div>
                    <div class="h-2 w-full rounded-full bg-white/60 dark:bg-zinc-800/80">
                        <div class="h-2 rounded-full bg-primary-500 transition-all duration-500 ease-out dark:bg-primary-400" style="width: {{ $usagePercent }}%;"></div>
                    </div>
                </div>
                @if ($monthly_proposals_remaining === 0)
                    <a href="{{ url('seller/projects/bids') }}" class="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary-600 px-4 py-2 text-sm font-semibold text-white shadow-md transition hover:-translate-y-0.5 hover:bg-primary-700">
                        <i class="ph ph-shopping-bag"></i>
                        @lang('messages.t_free_plan_limit_reached_action')
                    </a>
                @else
                    <p class="mt-4 text-[12px] text-slate-500 dark:text-slate-400">
                        @lang('messages.t_monthly_proposals_keep_bidding')
                    </p>
                @endif
            </div>
        @endif
    ...
    <div class="mx-auto grid max-w-6xl gap-8 xl:grid-cols-[minmax(0,1.7fr)_minmax(0,1fr)]">
        <div class="rounded-3xl border border-white/60 bg-white/95 p-6 shadow-sm backdrop-blur-sm dark:border-zinc-700 dark:bg-zinc-900/70">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                ...
                <a href="{{ url('seller/projects/awarded') }}" class="inline-flex items-center gap-2 rounded-full bg-primary-600 px-4 py-2 text-xs font-semibold text-white transition hover:bg-primary-700">
                    <i class="ph ph-list-checks"></i>
                    إدارة العقود
                </a>
            </div>

            @if ($latest_awarded_projects && $latest_awarded_projects->count())
                <div class="mt-6 divide-y divide-slate-200 dark:divide-zinc-800">
                    @foreach ($latest_awarded_projects as $project)
                        <article class="flex flex-col gap-4 py-5 first:pt-0 last:pb-0 md:flex-row md:items-center md:justify-between" wire:key="freelancer-dashboard-latest-projects-{{ $project->uid }}">
                            <div class="space-y-2">
                                <a href="{{ url('project/' . $project->pid . '/' . $project->slug) }}" class="text-base font-semibold text-slate-900 transition hover:text-primary-600 dark:text-white">{{ $project->title }}</a>
                                <div class="flex flex-wrap gap-2 text-[12px] text-slate-500 dark:text-slate-400">
                                    <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 dark:bg-zinc-800">{{ __('messages.t_budget') }}: {{ money($project->budget_min, settings('currency')->code, true) }} - {{ money($project->budget_max, settings('currency')->code, true) }}</span>
                                    <span class="inline-flex items-center gap-1 rounded-full bg-slate-100 px-2.5 py-1 dark:bg-zinc-800">{{ format_date($project->created_at, config('carbon-formats.F_d,_Y')) }}</span>
                                </div>
                            </div>
                            <div class="flex flex-col items-end gap-3 sm:flex-row sm:items-center">
                                <span class="inline-flex items-center gap-2 rounded-full bg-primary-50 px-3 py-1 text-xs font-semibold text-primary-600 dark:bg-primary-500/15 dark:text-primary-300">
                                    ...
                                    {{ money(optional($project->awarded_bid)->amount, settings('currency')->code, true) }}
                                </span>
                                @switch($project->status)
                                    @case('pending_final_review')
                                        <span class="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700 dark:bg-amber-500/10 dark:text-amber-300">{{ __('messages.t_pending_final_review') }}</span>
                                        @break
                                    @case('active')
                                        <span class="rounded-full bg-blue-100 px-3 py-1 text-xs font-semibold text-blue-700 dark:bg-blue-500/10 dark:text-blue-300">{{ __('messages.t_active') }}</span>
                                        @break
                                    @case('completed')
                                        <span class="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300">{{ __('messages.t_completed') }}</span>
                                        @break
                                    @case('incomplete')
                                        <span class="rounded-full bg-rose-100 px-3 py-1 text-xs font-semibold text-rose-700 dark:bg-rose-500/10 dark:text-rose-300">{{ __('messages.t_incomplete') }}</span>
                                        @break
                                    @case('under_development')
                                        <span class="rounded-full bg-purple-100 px-3 py-1 text-xs font-semibold text-purple-700 dark:bg-purple-500/10 dark:text-purple-300">{{ __('messages.t_under_development') }}</span>
                                        @break
                                    @case('closed')
                                        <span class="rounded-full bg-slate-200 px-3 py-1 text-xs font-semibold text-slate-700 dark:bg-slate-600/20 dark:text-slate-300">{{ __('messages.t_closed') }}</span>
                                        @break
                                    @default
                                @endswitch
                            </div>
        ...
        <div class="space-y-6">
            <div class="rounded-3xl border border-white/60 bg-white/95 p-6 shadow-sm backdrop-blur-sm dark:border-zinc-700 dark:bg-zinc-900/70">
                ...
                <div class="mt-5 space-y-4">
                    @forelse ($latest_messages as $message)
                        <a href="{{ url('messages', $message['uid']) }}" class="group flex items-start gap-3 rounded-2xl border border-slate-200/70 px-4 py-3 transition hover:-translate-y-0.5 hover:border-primary-200 hover:bg-primary-50/40 dark:border-zinc-700 dark:hover:border-primary-400/40 dark:hover:bg-primary-500/10">
                            <img src="{{ $message['avatar'] }}" alt="{{ $message['username'] }}" class="h-10 w-10 rounded-full object-cover" />
                            <div class="space-y-1">
                                <div class="flex items-center gap-2">
                                    <span class="text-sm font-semibold text-slate-900 dark:text-white">{{ $message['username'] }}</span>
                                ...
                                <p class="text-[13px] text-slate-500 line-clamp-2 dark:text-slate-400">{{ $message['message']['body'] }}</p>
                            </div>
                        </a>
                    @empty
            ...
            <div class="rounded-3xl border border-white/60 bg-white/95 p-6 shadow-sm backdrop-blur-sm dark:border-zinc-700 dark:bg-zinc-900/70">
                ...
                <ul class="mt-4 space-y-3 text-sm">
                    <li>
                        <a href="{{ url('seller/projects') }}" class="flex items-center justify-between rounded-2xl border border-slate-200 px-4 py-3 transition hover:border-primary-200 hover:bg-primary-50/40 dark:border-zinc-700 dark:hover:border-primary-500/30 dark:hover:bg-primary-500/10">
                    ...
                    <li>
                        <a href="{{ url('seller/offers') }}" class="flex items-center justify-between rounded-2xl border border-slate-200 px-4 py-3 transition hover:border-primary-200 hover:bg-primary-50/40 dark:border-zinc-700 dark:hover:border-primary-500/30 dark:hover:bg-primary-500/10">
                            <span class="flex items-center gap-2 text-slate-700 dark:text-slate-200">
                    ...
                    <li>
                        <a href="{{ url('account/settings') }}" class="flex items-center justify-between rounded-2xl border border-slate-200 px-4 py-3 transition hover:border-primary-200 hover:bg-primary-50/40 dark:border-zinc-700 dark:hover:border-primary-500/30 dark:hover:bg-primary-500/10">
                            <span class="flex items-center gap-2 text-slate-700 dark:text-slate-200">
