                                ...
                                @foreach ($categories as $cat)
                                    <label class="group flex justify-between items-center text-sm rtl:space-x-reverse">
                                        <div class="flex items-center space-x-2 rtl:space-x-reverse">
                                            <input type="checkbox" wire:model.live="cats" value="{{ $cat->slug }}"
                                                wire:key="cat-{{ $cat->id }}" class="h-4 w-4 text-primary-600 rounded border-gray-300
                ...
                <div class="rounded-3xl border border-gray-200 bg-white/80 p-5 shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/5"
                    ...
                    <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                        <div class="text-sm font-semibold text-gray-700 dark:text-gray-200">
                            عرض {{ number_format($projects->count()) }} من {{ number_format($totalProjects) }} مشاريع
                            متاحة
                        </div>
                            ...
                            <div class="flex flex-wrap gap-3 text-xs">
                                @if ($selectedCategories->count())
                                    <span
                                        ...
                                        {{ $selectedCategories->take(2)->implode('، ') }}@if($selectedCategories->count() > 2)
                                        {{ ' +' . ($selectedCategories->count() - 2) }} @endif
                                    </span>
                                @endif
                                ...
                                @if (!empty($budget_min) || !empty($budget_max))
                                    <span
                                        ...
                                        الميزانية: {{ $budget_min ?: '0' }} - {{ $budget_max ?: '∞' }}
                                    </span>
                                @endif
                                ...
                                @if (!empty($date))
                                    <span
                                        ...
                                        {{ $date == 1 ? 'آخر 24 ساعة' : ($date == 7 ? 'آخر 7 أيام' : 'آخر 30 يوماً') }}
                                    </span>
                                @endif
