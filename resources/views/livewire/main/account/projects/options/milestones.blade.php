<div class="max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-8 mt-[7rem] py-12 lg:pt-16 lg:pb-24">
        ...
        <div class="lg:col-span-9 lg:ltr:ml-8 lg:rtl:mr-8">
                ...
                <div class="mx-auto max-w-7xl">
                    <div class="lg:flex lg:items-center lg:justify-between">
                        ...
                        <div class="mt-5 flex justify-between lg:mt-0 lg:ltr::ml-4 lg:rtl:mr-4">
                            ...
                            @if ($canRateFreelancer)
                                <span class="block sm:ltr:ml-3 sm:rtl:mr-3">
                                    <button type="button"
                                        wire:click="openRatingModal"
                                        class="inline-flex items-center rounded-sm border border-transparent bg-primary-600 px-4 py-2 text-[13px] font-semibold text-white shadow-sm hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-zinc-800">
                                        <i class="ph-duotone ph-star text-base ltr:mr-2 rtl:ml-2"></i>
                                        @lang('messages.t_rate_freelancer')
                                    </button>
                                </span>
                            @elseif ($clientReview)
                                <span class="block sm:ltr:ml-3 sm:rtl:mr-3">
                                    <span class="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 dark:border-emerald-900/40 dark:bg-emerald-900/30 dark:text-emerald-200">
                                        <i class="ph-duotone {{ $clientReview->is_skipped ? 'ph-arrow-bend-up-left' : 'ph-check-circle' }} text-sm"></i>
                                        {{ $clientReview->is_skipped ? __('messages.t_rating_skipped_badge') : __('messages.t_rating_submitted_badge') }}
                                    </span>
                                </span>
                            @endif
                        </div>
            ...
            <div class="w-full">
                ...
                <div class="w-full mb-6">
                    ...
                    <div class="mt-8 space-y-10">
                        @forelse ($timeline as $entry)
                            @php
                                ...
                                $canFollowUp = $milestone->status === 'paid' && in_array($project->status, ['completed', 'pending_final_review']);
                            @endphp

                            <div class="relative ltr:pl-14 rtl:pr-14" wire:key="timeline-root-{{ $milestone->uid }}">
                                @if (! $loop->last)
                                ...
                                <div class="rounded-2xl border border-slate-200/80 bg-white px-6 py-6 shadow-sm dark:border-zinc-700 dark:bg-zinc-800">
                                    ...
                                    <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4 text-sm">
                                        <div>
                                            <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 dark:text-zinc-500">@lang('messages.t_paid_to_freelancer')</p>
                                        ...
                                        <div>
                                            <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 dark:text-zinc-500">@lang('messages.t_milestone_employer_fee_name')</p>
                                            <p class="mt-1 font-semibold text-slate-800 dark:text-zinc-100">{{ money(convertToNumber($milestone->employer_commission), settings('currency')->code, true) }}</p>
                                        </div>
                                        <div>
                                            <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 dark:text-zinc-500">@lang('messages.t_total')</p>
                                            <p class="mt-1 font-semibold text-slate-800 dark:text-zinc-100">{{ money(convertToNumber($milestone->amount) + convertToNumber($milestone->employer_commission), settings('currency')->code, true) }}</p>
                                        </div>
                                    ...
                                    <div class="mt-6 flex flex-wrap items-center justify-end gap-2">
                                        <button
                                            wire:click="details('{{ $milestone->uid }}')"
                                            type="button"
                                            aria-label="{{ __('messages.t_details') }}"
                                            class="inline-flex h-9 w-9 items-center justify-center rounded border border-slate-200 bg-white text-slate-600 shadow-sm transition-all hover:border-slate-300 hover:bg-slate-50 hover:text-slate-700 focus:outline-none focus:ring focus:ring-slate-400/40 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700"
                                            data-tooltip-target="timeline-details-{{ $milestone->uid }}">
                                        ...
                                        <x-forms.tooltip id="timeline-details-{{ $milestone->uid }}" :text="__('messages.t_details')" />

                                        @if ($canFollowUp)
                                            <button
                                                wire:click="openFollowUp('{{ $milestone->uid }}')"
                                                type="button"
                                                aria-label="{{ __('messages.t_create_follow_up_milestone') }}"
                                                class="inline-flex h-9 w-9 items-center justify-center rounded border border-primary-200 bg-white text-primary-600 shadow-sm transition-all hover:border-primary-400 hover:bg-primary-50 hover:text-primary-700 focus:outline-none focus:ring focus:ring-primary-400/40 dark:border-primary-400/40 dark:bg-zinc-800 dark:text-primary-100 dark:hover:bg-zinc-700"
                                                data-tooltip-target="timeline-follow-{{ $milestone->uid }}">
                                                <i class="ph-duotone ph-plus-circle text-lg"></i>
                                            </button>
                                            <x-forms.tooltip id="timeline-follow-{{ $milestone->uid }}" :text="__('messages.t_create_follow_up_milestone')" />
                                        @endif

                                        @if ($milestone->status === 'request')
                                            <button
                                                wire:click="confirmPay('{{ $milestone->uid }}')"
                                                type="button"
                                                aria-label="{{ __('messages.t_deposit_funds') }}"
                                                class="inline-flex h-9 w-9 items-center justify-center rounded border border-slate-200 bg-white text-slate-600 shadow-sm transition-all hover:border-slate-300 hover:bg-slate-50 hover:text-slate-700 focus:outline-none focus:ring focus:ring-slate-400/40 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700"
                                                data-tooltip-target="timeline-pay-{{ $milestone->uid }}">
                                            ...
                                            <x-forms.tooltip id="timeline-pay-{{ $milestone->uid }}" :text="__('messages.t_deposit_funds')" />
                                        @endif

                                        @if ($milestone->status === 'funded')
                                            <button
                                                wire:click="confirmRelease('{{ $milestone->uid }}')"
                                                type="button"
                                                aria-label="{{ __('messages.t_release') }}"
                                                class="inline-flex h-9 w-9 items-center justify-center rounded border border-slate-200 bg-white text-slate-600 shadow-sm transition-all hover:border-slate-300 hover:bg-slate-50 hover:text-slate-700 focus:outline-none focus:ring focus:ring-emerald-400/40 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700"
                                                data-tooltip-target="timeline-release-{{ $milestone->uid }}">
                                            ...
                                            <x-forms.tooltip id="timeline-release-{{ $milestone->uid }}" :text="__('messages.t_release')" />
                                        @endif
                                    </div>
                                        ...
                                        <div class="mt-7 space-y-6 border-l border-dashed border-slate-200/80 ltr:pl-8 rtl:pr-8 dark:border-zinc-600/70">
                                            @foreach ($entry['children'] as $childEntry)
                                                @php
                                                    ...
                                                    $childCanFollowUp = $child->status === 'paid' && in_array($project->status, ['completed', 'pending_final_review']);
                                                @endphp

                                                <div class="relative ltr:pl-10 rtl:pr-10" wire:key="timeline-child-{{ $child->uid }}">
                                                    ...
                                                    <div class="rounded-xl border border-slate-200/80 bg-slate-50 px-5 py-5 shadow-sm dark:border-zinc-700 dark:bg-zinc-900/60">
                                                        ...
                                                        <div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4 text-sm">
                                                            ...
                                                            <div>
                                                                <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 dark:text-zinc-500">@lang('messages.t_milestone_employer_fee_name')</p>
                                                                <p class="mt-1 font-semibold text-slate-800 dark:text-zinc-100">{{ money(convertToNumber($child->employer_commission), settings('currency')->code, true) }}</p>
                                                            </div>
                                                            <div>
                                                                <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 dark:text-zinc-500">@lang('messages.t_total')</p>
                                                                <p class="mt-1 font-semibold text-slate-800 dark:text-zinc-100">{{ money(convertToNumber($child->amount) + convertToNumber($child->employer_commission), settings('currency')->code, true) }}</p>
                                                            </div>
                                                            <div class="lg:text-right rtl:lg:text-left">
                                                                <p class="text-[11px] font-semibold uppercase tracking-wide text-slate-400 dark:text-zinc-500">@lang('messages.t_id')</p>
                                                        ...
                                                        <div class="mt-5 flex flex-wrap items-center justify-end gap-2">
                                                            <button
                                                                wire:click="details('{{ $child->uid }}')"
                                                                type="button"
                                                                aria-label="{{ __('messages.t_details') }}"
                                                                class="inline-flex h-8 w-8 items-center justify-center rounded border border-slate-200 bg-white text-slate-600 shadow-sm transition-all hover:border-slate-300 hover:bg-slate-50 hover:text-slate-700 focus:outline-none focus:ring focus:ring-slate-400/40 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700"
                                                                data-tooltip-target="timeline-details-{{ $child->uid }}">
                                                                <svg class="h-4 w-4" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M20 2H4c-1.103 0-2 .897-2 2v12c0 1.103.897 2 2 2h3v3.767L13.277 18H20c1.103 0 2-.897 2-2V4c0-1.103-.897-2-2-2zm0 14h-7.277L9 18.233V16H4V4h16v12z"></path><path d="M7 7h10v2H7zm0 4h7v2H7z"></path></svg>
                                                            </button>
                                                            <x-forms.tooltip id="timeline-details-{{ $child->uid }}" :text="__('messages.t_details')" />

                                                            @if ($childCanFollowUp)
                                                                <button
                                                                    wire:click="openFollowUp('{{ $child->uid }}')"
                                                                    type="button"
                                                                    aria-label="{{ __('messages.t_create_follow_up_milestone') }}"
                                                                    class="inline-flex h-8 w-8 items-center justify-center rounded border border-primary-200 bg-white text-primary-600 shadow-sm transition-all hover:border-primary-400 hover:bg-primary-50 hover:text-primary-700 focus:outline-none focus:ring focus:ring-primary-400/40 dark:border-primary-400/40 dark:bg-zinc-800 dark:text-primary-100 dark:hover:bg-zinc-700"
                                                                    data-tooltip-target="timeline-follow-{{ $child->uid }}">
                                                                    <i class="ph-duotone ph-plus-circle text-base"></i>
                                                                </button>
                                                                <x-forms.tooltip id="timeline-follow-{{ $child->uid }}" :text="__('messages.t_create_follow_up_milestone')" />
                                                            @endif

                                                            @if ($child->status === 'request')
                                                                <button
                                                                    wire:click="confirmPay('{{ $child->uid }}')"
                                                                    type="button"
                                                                    aria-label="{{ __('messages.t_deposit_funds') }}"
                                                                    class="inline-flex h-8 w-8 items-center justify-center rounded border border-slate-200 bg-white text-slate-600 shadow-sm transition-all hover:border-slate-300 hover:bg-slate-50 hover:text-slate-700 focus:outline-none focus:ring focus:ring-slate-400/40 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700"
                                                                    data-tooltip-target="timeline-pay-{{ $child->uid }}">
                                                                    <svg class="h-4 w-4" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 15c-1.84 0-2-.86-2-1H8c0 .92.66 2.55 3 2.92V18h2v-1.08c2-.34 3-1.63 3-2.92 0-1.12-.52-3-4-3-2 0-2-.63-2-1s.7-1 2-1 1.39.64 1.4 1h2A3 3 0 0 0 13 7.12V6h-2v1.09C9 7.42 8 8.71 8 10c0 1.12.52 3 4 3 2 0 2 .68 2 1s-.62 1-2 1z"></path><path d="M5 2H2v2h2v17a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1V4h2V2H5zm13 18H6V4h12z"></path></svg>
                                                                </button>
                                                                <x-forms.tooltip id="timeline-pay-{{ $child->uid }}" :text="__('messages.t_deposit_funds')" />
                                                            @endif

                                                            @if ($child->status === 'funded')
                                                                <button
                                                                    wire:click="confirmRelease('{{ $child->uid }}')"
                                                                    type="button"
                                                                    aria-label="{{ __('messages.t_release') }}"
                                                                    class="inline-flex h-8 w-8 items-center justify-center rounded border border-slate-200 bg-white text-slate-600 shadow-sm transition-all hover:border-slate-300 hover:bg-slate-50 hover:text-slate-700 focus:outline-none focus:ring focus:ring-emerald-400/40 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-200 dark:hover:bg-zinc-700"
                                                                    data-tooltip-target="timeline-release-{{ $child->uid }}">
                                                                    <svg class="h-4 w-4" stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="m10 15.586-3.293-3.293-1.414 1.414L10 18.414l9.707-9.707-1.414-1.414z"></path></svg>
                                                                </button>
                                                                <x-forms.tooltip id="timeline-release-{{ $child->uid }}" :text="__('messages.t_release')" />
                                                            @endif
                                                        </div>
                                                    </div>
                                                </div>
                    ...
                    {{-- Table --}}
                    <div class="hidden mt-8 overflow-x-auto overflow-y-hidden scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100 dark:scrollbar-thumb-zinc-800 dark:scrollbar-track-zinc-600">
                                ...
                                @forelse ($payments as $p)
                                    <tr class="intro-x shadow-sm bg-white dark:bg-zinc-800 rounded-md h-16" wire:key="employer-dashboard-project-milestones-{{ $p->uid }}">
                                        ...
                                        <td class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md w-64 rtl:text-right">
                                            <div class="space-y-2">
                                                @if ($p->is_follow_up)
                                                    <span class="inline-flex items-center gap-2 rounded-full bg-amber-100 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wider text-amber-700 dark:bg-amber-500/10 dark:text-amber-200">
                                                        <i class="ph-duotone ph-repeat text-xs"></i>
                                                        @lang('messages.t_follow_up_badge')
                                                    </span>
                                                @endif
                                                <div class="text-gray-500 leading-relaxed dark:text-gray-100 text-[13px] font-normal break-words flex-none line-clamp-2">
                                                    {{ $p->description }}
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-5 py-3 first:ltr:rounded-l-md last:ltr:rounded-r-md first:rtl:rounded-r-md last:rtl:rounded-l-md text-center">
                                            ...
                                            <button wire:click="details('{{ $p->uid }}')" type="button" aria-label="{{ __('messages.t_details') }}" class="inline-flex justify-center items-center border font-semibold focus:outline-none w-8 h-8 text-sm rounded border-gray-300 bg-white text-gray-600 shadow-sm hover:text-gray-800 hover:bg-gray-100 hover:border-gray-300 hover:shadow focus:ring focus:ring-gray-500 focus:ring-opacity-25 active:bg-white active:border-white active:shadow-none transition-all duration-200 dark:bg-zinc-700 dark:border-transparent dark:text-zinc-200 dark:hover:bg-zinc-600" data-tooltip-target="tooltip-actions-details-{{ $p->uid }}">
                                                ...
                                                {{-- Follow up --}}
                                                @if ($p->status === 'paid' && in_array($project->status, ['completed', 'pending_final_review']))
                                                    <button wire:click="openFollowUp('{{ $p->uid }}')" type="button" aria-label="{{ __('messages.t_create_follow_up_milestone') }}" class="inline-flex justify-center items-center border font-semibold focus:outline-none w-8 h-8 text-sm rounded border-primary-200 bg-white text-primary-600 shadow-sm hover:text-primary-700 hover:bg-primary-50 hover:border-primary-400 hover:shadow focus:ring focus:ring-primary-500 focus:ring-opacity-25 active:bg-white active:border-primary-500 active:shadow-none transition-all duration-200 dark:bg-zinc-700 dark:border-zinc-500 dark:text-primary-200 dark:hover:bg-zinc-600" data-tooltip-target="tooltip-actions-follow-up-{{ $p->uid }}">
                                                        <i class="ph-duotone ph-plus-circle text-lg"></i>
                                                    </button>
                                                    <x-forms.tooltip id="tooltip-actions-follow-up-{{ $p->uid }}" :text="__('messages.t_create_follow_up_milestone')" />
                                                @endif
    ...
    <x-forms.modal id="modal-project-review" target="modal-project-review-trigger" uid="modal_project_review" placement="center-center" size="max-w-xl">

        <x-slot name="title">
            <div class="space-y-1">
                <h2 class="text-base font-bold tracking-wide text-slate-800 dark:text-zinc-100">
                    @lang('messages.t_rate_freelancer_title', ['name' => $project->awarded_bid?->user?->fullname ?: $project->awarded_bid?->user?->username])
                </h2>
                <p class="text-sm text-slate-500 dark:text-zinc-300">@lang('messages.t_rate_freelancer_subtitle')</p>
            </div>
        </x-slot>
        ...
        <x-slot name="content">
            <div class="space-y-6">
                    ...
                    <div class="flex items-center justify-center gap-2">
                        @for ($i = 1; $i <= 5; $i++)
                            <label class="cursor-pointer">
                                <input type="radio" class="sr-only" name="ratingScore" value="{{ $i }}" wire:model="ratingScore">
                                <i class="ph-duotone ph-star text-3xl transition-colors duration-150 {{ $ratingScore >= $i ? 'text-amber-400' : 'text-slate-300 dark:text-zinc-600' }}"></i>
                            </label>
                        @endfor
                    </div>
                    @error('ratingScore')
                ...
                <div>
                    <label for="project-review-comment" class="block text-sm font-semibold text-slate-600 dark:text-zinc-200 mb-2">
                        @lang('messages.t_optional_feedback_label')
                    </label>
                    <textarea id="project-review-comment" rows="4" wire:model.defer="ratingComment" class="block w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 shadow-sm transition focus:border-primary-500 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-opacity-40 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100" placeholder="{{ __('messages.t_optional_feedback_placeholder') }}"></textarea>
        ...
        <x-slot name="footer">
            <div class="flex flex-col-reverse sm:flex-row sm:justify-end sm:space-x-3 sm:space-x-reverse w-full">
                ...
                <button type="button"
                    ...
                    class="inline-flex justify-center items-center rounded border font-semibold focus:outline-none px-4 py-2 leading-5 text-xs tracking-wide border-transparent bg-primary-600 text-white shadow-sm hover:bg-primary-700 focus:ring focus:ring-primary-500 focus:ring-opacity-25 disabled:opacity-60 disabled:cursor-not-allowed">
